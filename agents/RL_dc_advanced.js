...
class EnhancedDataCenterEnvironment {
    constructor(agentId, socket) {
        ...
        //this.hvac = new HVAC("HVAC1");
        this.hvacs = Array(config.environment.hvacCount).fill(0).map((_, i) =>
            new HVAC(
                `HVAC${i + 1}`
            )
        );
    ...
    updateRackTemperatures() {
        ...
        this.rackTemperatures.forEach((rack, i) => {
            ...
            const heatGain = this.rackPowerDraws[i].get() * 0.08; // 0.08°C per kW

            // Cooling effect (can't exceed heat gain + ambient)
            const maxCooling = heatGain + (rack.value - ambientTemp) * 0.05;
            const coolingEffect = Math.min(
                maxCooling,
                (this.fanSpeed.get() / 100) * config.environment.maxCoolingPower * 0.02
            );

            // Update temperature (never < ambient) - can go over MAX!!
            rack.value = Math.max(
                ambientTemp,
    ...
    convertToBooleanOrFloat(value) {
        ...
        // Keep booleans/numbers as-is, discard others
      ...
      convertData(data) {
        const result = {};
          for (const key in data) {
            result[key] = this.convertToBooleanOrFloat(data[key]);
          }
          return result;
      }
    ...
    emitStatusUpdate(action) {
        ...
        let status = {
            from: this.agentId,
            data: {
                powerRackStatus: this.rackPower,
                ...
                fanSpeed: this.fanSpeed.get().toFixed(1),
                airflow: this.airflow.get().toFixed(1),
                pue: this.pue.get().toFixed(3),
                failureRisk: this.failureRisk.toFixed(2),
        ...
        this.socket.emit("message", {
            from: this.agentId,
            to: "waylay_agent",
            data: { request: "storeData", resource: this.agentId, data: this.convertData(status.data) }
        });
...
class EnhancedSimulation {
    ...
    setupSocketListeners() {
        ...
        this.socket.on("message", (msg) => {
            if (msg.data?.value && msg.data?.request === "setWorkload") {
                console.log("set workload" , msg.data.value);
                this.env.workload.value = msg.data.value;
            } else if (msg.data?.value && msg.data?.request === "setTemperature") {
                console.log("set target temperature" , msg.data.value);
                this.env.targetTemperature.value = msg.data.value;
            } else if (msg.data?.request === "powerCut") {
                console.log("powerCut");
                this.env.rackPowerDraws.forEach(rack => rack.value =0 );
                this.env.rackPower = false;
            } else if (msg.data?.request === "powerOn") {
                console.log("powerOn");
                this.env.rackPowerDraws.forEach(rack => rack.value =0.5 );
                this.env.rackPower = true;
            }
